...
module.exports = {
    name: "herkesi teleportlar",
    description: "herkesi tpler",
    role: "god",

    options: [
        {
            type: "SUB_COMMAND",
            name: "kordinat gir aq salagi",
            description: "kordinat gir",
            options: [
                {
                    name: "x",
        ...
        {
            type: "SUB_COMMAND",
            name: "önceden belirlenmiş yerler",
    ...
    run: async (client, interaction, args) => {
        ...
        if (args.coords) {
            ...
            return interaction.reply({ content: "herkes ısınlandı.", ephemeral: false });
        } else if (args.preset) {
            teleportEveryone(locations[args.location][0], locations[args.location][1], locations[args.location][2]);
            client.utils.log.info(`[${interaction.member.displayName}] herkes buraya teleportlandı  ${args.location}`);
            return interaction.reply({ content: `herkes buraya ışınlandı ${args.location}`, ephemeral: false });
        }
    },
};
